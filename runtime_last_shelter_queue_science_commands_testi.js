...
(async()=>{
  ...
  assert.strictEqual(
    queues[0].computedTypeName,
    "SCIENCE"
  );
  assert.strictEqual(
    queues[0].computedState,
    "OUT_SYN"
  );
  assert.strictEqual(
    queues[0].isFree,
  ...
  assert.deepStrictEqual(
    catalog.map(x=>x.itemId),
    ["901000","901100","901200","901300"]
  );
  ...
  console.log(
    "PASS: verified Last Shelter queue/science read runtime is wired without inventing unverified resource-code debit semantics."
  );
})().catch(error=>{
  console.error(error);
  process.exitCode=1;
