...
function formasiyaStateTeminEt(konvoy) {
  if (!konvoy || typeof konvoy !== "object") {
    throw new Error("Konvoy formasiya state-i üçün konvoy yoxdur.");
  }

  if (!konvoy.formasiya || typeof konvoy.formasiya !== "object" || Array.isArray(konvoy.formasiya)) {
    konvoy.formasiya = {};
  }

  konvoy.formasiya.version = 1;

  if (!Array.isArray(konvoy.formasiya.siralar)) {
  ...
  konvoy.formasiya.siralar = FORMASIYA_SIRA_IDLERI.map(
    siraId => movcud.get(siraId) || bosSira(siraId)
  );
...
function legacyQosunlardanFormasiyaHazirla(qosunlar) {
  ...
  // Legacy model sıra məlumatı saxlamırdı. Yalnız üç və daha az unit tipi
  // olduqda təhlükəsiz deterministik migration edə bilirik.
  if (entries.length > FORMASIYA_SIRA_IDLERI.length) {
    return null;
  }
...
function konvoyFormasiyalariniTeminEt(state) {
  ...
  for (const konvoy of konvoylar.items) {
    ...
    // Köhnə snapshot-da yalnız `qosunlar` varsa, mümkün olduqda bir dəfə
    // 3 sıra formasiyasına təhlükəsiz çevrilir. 3-dən çox unit tipi varsa
    // məlumat itirməmək üçün legacy state toxunulmadan qalır.
    if (
      Object.keys(formasiyaQosunlari).length === 0 &&
      konvoy.qosunlar &&
      Object.keys(konvoy.qosunlar).length > 0
    ) {
      const migrated = legacyQosunlardanFormasiyaHazirla(konvoy.qosunlar);
      if (migrated) {
        formasiya.siralar = migrated;
      }
    }
  }
...
function formasiyaTeyinEt(state, konvoyId, rawSiralar) {
  ...
  if (!Array.isArray(rawSiralar)) {
    return { success: false, message: "Konvoy formasiyası düzgün göndərilməyib." };
  }

  const rawById = new Map();
  for (const raw of rawSiralar) {
    const siraId = metnAl(raw && raw.siraId, 32);
    if (!FORMASIYA_SIRA_IDLERI.includes(siraId)) {
      return { success: false, message: "Naməlum formasiya sırası göndərilib." };
    }
    if (rawById.has(siraId)) {
      return { success: false, message: "Eyni formasiya sırası iki dəfə göndərilib." };
    }
    rawById.set(siraId, raw);
  }

  const siralar = FORMASIYA_SIRA_IDLERI.map(
    siraId => siraniTemizle(rawById.get(siraId), siraId)
  );
  const qosunlar = formasiyaQosunlariniTopla(siralar);

  // Mövcud server-authoritative army/reservation/capacity validatorunu saxlayırıq.
  // Capacity mənbəyi ayrıca yeni formula moduluna keçiriləndə bu çağırış da
  // avtomatik həmin yeni validatoru istifadə edəcək.
