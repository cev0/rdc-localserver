...
function aktivKonvoyIdSetiniAl(state) {
  ...
  if (vahidEmeliyyatlar && typeof vahidEmeliyyatlar === "object" && !Array.isArray(vahidEmeliyyatlar)) {
    for (const [rawConvoyId, emeliyyat] of Object.entries(vahidEmeliyyatlar)) {
      ...
      if (!convoyId || !status || status === "idle") continue;
      aktiv.add(convoyId);
    }
  }

  const legacyToplama = state && state.xeriteToplama && state.xeriteToplama.activeByConvoy;
  if (legacyToplama && typeof legacyToplama === "object" && !Array.isArray(legacyToplama)) {
  ...
  const legacyDoyus = state && state.worldEnemyBattle && state.worldEnemyBattle.activeByConvoy;
  if (legacyDoyus && typeof legacyDoyus === "object" && !Array.isArray(legacyDoyus)) {
...
function aktivKonvoyQosunlariniAl(state) {
  ...
  for (const konvoy of konvoySiyahisiniAl(state)) {
    ...
    for (const [unitId, say] of Object.entries(qosunObyektiniTemizle(konvoy && konvoy.qosunlar))) {
...
function mudafieQosunlariniHazirla(state, nowMs = Date.now()) {
  const mudafieKonvoylari = mudafiedeIstirakEdenKonvoyIdSetiniAl(state, nowMs);
  const mudafie = {};

  for (const konvoy of konvoySiyahisiniAl(state)) {
    const convoyId = metnAl(konvoy && konvoy.konvoyId, 64);
    if (!convoyId || !mudafieKonvoylari.has(convoyId)) continue;

    for (const [unitId, say] of Object.entries(qosunObyektiniTemizle(konvoy && konvoy.qosunlar))) {
      mudafie[unitId] = (mudafie[unitId] || 0) + say;
    }
  }
...
function mudafieQehremanIdleriniHazirla(state, nowMs = Date.now()) {
  const mudafieKonvoylari = mudafiedeIstirakEdenKonvoyIdSetiniAl(state, nowMs);
  const netice = [];
...
function pvpDoyusSnapshotQaydasiniHazirla() {
  return {
    ...
    defenseSource: "selected_idle_convoys_only",
    freeTroopsAutoDefend: false,
    defenseDisabledConvoyExcluded: true,
    activeMissionConvoyExcluded: true,
    selectedIdleConvoyTroopsIncluded: true,
    selectedIdleConvoyHeroesIncluded: true,
...
function pvpHucumcuSnapshotiniHazirla(state, convoyId, nowMs = Date.now()) {
  ...
  if (!konvoy || konvoy.aciqdir !== true) {
    return { success: false, message: "PvP hücum snapshot-u üçün açıq konvoy tapılmadı." };
  }
  ...
  if (troopCount <= 0) {
    return { success: false, message: "PvP hücum snapshot-u üçün konvoyda qoşun yoxdur." };
  }

  const heroIds = Array.isArray(konvoy.qehremanIdleri)
    ? konvoy.qehremanIdleri.map(heroId => metnAl(heroId, 128)).filter(Boolean)
    : [];
...
function pvpMudafieciSnapshotiniHazirla(state, playerId, nowMs = Date.now()) {
  if (!state || typeof state !== "object") {
    return { success: false, message: "PvP müdafiə snapshot-u üçün oyunçu state-i yoxdur." };
  }

  const qosunlar = mudafieQosunlariniHazirla(state, nowMs);
  const heroIds = mudafieQehremanIdleriniHazirla(state, nowMs);
  const troopCount = qosunSayiniHesabla(qosunlar);
  const defenseConvoyIds = Array.from(
    mudafiedeIstirakEdenKonvoyIdSetiniAl(state, nowMs)
  ).sort();
  ...
  return {
    ...
    snapshot: {
      ...
      troops: qosunlar,
      formation: qosundanDeterministikFormasiyaHazirla(qosunlar, "mudafie"),
      heroIds,
      troopCount,
      troopPower: qosunGucunuHesabla(qosunlar),
      heroPowerApplied: false,
