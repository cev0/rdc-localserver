...
async function pinBerpaKoduEmailiGonder(email, kod) {
  ...
  if (EMAIL_DEV_LOG_CODE) {
    console.log("[EMAIL_DEV] PIN bərpa kodu:", {
      kod: temizKod
    });

    return {
      success: true,
      message: "DEV rejimi: PIN bərpa kodu server loguna yazıldı.",
      emailId: "dev-test"
    };
  }
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;background:#11140f;color:#ffffff;padding:32px;">
      <h2 style="margin:0 0 20px 0;color:#d8aa56;">
        Təhlükəsizlik PIN-i bərpası
      </h2>

      <p>
        Oyun hesabınızın təhlükəsizlik PIN-ini yeniləmək üçün aşağıdakı kodu daxil edin:
      </p>
      ...
      <p style="color:#777777;font-size:12px;margin-top:30px;">
        Əgər PIN bərpa sorğusunu siz etməmisinizsə, bu məktubu nəzərə almayın.
      </p>
    </div>
  `;
  ...
  try {
    const cavab = await fetch(
      "https://api.resend.com/emails",
      {
        ...
        body: JSON.stringify({
          from: EMAIL_FROM,
          to: [temizEmail],
          subject: "Təhlükəsizlik PIN-i bərpa kodu",
          html
        }),
    ...
    if (cavab.ok) {
      return {
        success: true,
        message: "PIN bərpa kodu e-poçta göndərildi.",
        emailId: json && json.id ? json.id : ""
      };
    }

    console.error("[PIN_BERPA_EMAIL] Resend xətası:", {
    ...
    return {
      success: false,
      message: "E-poçt xidməti PIN bərpa məktubunu göndərə bilmədi."
    };
  }
  catch (xeta) {
    console.error("[PIN_BERPA_EMAIL] Göndərmə xətası:", xeta);
