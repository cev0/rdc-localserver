...
function buildingReferenceProjectionHazirla() {
    ...
    record: {
      ...
      todayTaskCountLimit:
        nonNegativeInt(
          ...
          LAST_SHELTER_HELICOPTER.recordDefaults.todayTaskCountLimit
        )
    },
...
function worldProjectionHazirla(state) {
  ...
  return {
    maxstamina: nonNegativeInt(world.maxstamina, 100),
    lyt: nonNegativeInt(world.lyt, 0),
    stamina: nonNegativeInt(world.stamina, 100),
    enemy: Array.isArray(world.enemy) ? clone(world.enemy) : [],
    cityDefValue: nonNegativeInt(world.cityDefValue, 500),
    m: Array.isArray(world.marches) ? clone(world.marches) : [],
    point: world.point == null ? 0 : finiteInt(world.point, 0),
    ft: nonNegativeInt(world.ft, 0),
    userActMarchCntPerDay: nonNegativeInt(world.userActMarchCntPerDay, 0),
    autoIncrDefLimitValue: nonNegativeInt(world.autoIncrDefLimitValue, 0),
    sheildCdTime: nonNegativeInt(world.sheildCdTime, 0),
    lastStaminaTime:
      world.lastStaminaTime == null
        ? LONG_MAX_STRING
        : String(world.lastStaminaTime),
    gridType: nonNegativeInt(world.gridType, 2),
    lastCityDefTime: nonNegativeInt(world.lastCityDefTime, 0)
  };
}
...
function sevenDaysProjectionHazirla(state, uid = "") {
  ...
  return {
    reward: clone(LAST_SHELTER_SEVEN_DAYS_REWARD),
    taskInfo: clone(LAST_SHELTER_SEVEN_DAYS_TASK_INFO),
    activityInfo: {
      finishFlg: nonNegativeInt(runtime.finishFlg, 0),
      uid: String(uid || ""),
      taskCount: nonNegativeInt(runtime.taskCount, 100),
      count: nonNegativeInt(runtime.count, 0),
      startTime: nonNegativeInt(runtime.startTime, 0),
      unlockFlg: nonNegativeInt(runtime.unlockFlg, 1),
      endTime: nonNegativeInt(runtime.endTime, 0),
      type: nonNegativeInt(runtime.type, 49)
    }
  };
}
...
function repayProjectionHazirla(state) {
  ...
  return {
    startTime: LAST_SHELTER_REPAY_REFERENCE.observedWindow.startTime,
    payPoint: nonNegativeInt(runtime.payPoint,0),
    endTime: LAST_SHELTER_REPAY_REFERENCE.observedWindow.endTime,
...
function auxiliaryProjectionHazirla(state) {
  ...
  const lotteryRuntime = runtime.lottery && typeof runtime.lottery === "object"
    ? runtime.lottery : {};
  const cargoRuntime = runtime.cargo && typeof runtime.cargo === "object"
    ? runtime.cargo : {};

  return {
    killWorldBossNumber: nonNegativeInt(runtime.killWorldBossNumber,0),
    heroprison: Array.isArray(runtime.heroprison) ? clone(runtime.heroprison) : [],
    chatShield: Array.isArray(runtime.chatShield) ? clone(runtime.chatShield) : [],
    hasPassword: runtime.hasPassword === true,
    isOpenedKingdomAct: runtime.isOpenedKingdomAct === true,
    kingdomSeasonObj: clone(runtime.kingdomSeasonObj || {riseInfo:[]}),
    city_def_recover_record: nonNegativeInt(runtime.cityDefRecoverRecord,0),
    mail_translation: runtime.mailTranslation === true,
    killActivityBossNumber: nonNegativeInt(runtime.killActivityBossNumber,0),
    activationStoptime: nonNegativeInt(runtime.activationStoptime,0),
    exchange_gift: Array.isArray(runtime.exchangeGift) ? clone(runtime.exchangeGift) : [],
    resourcePoints: Array.isArray(runtime.resourcePoints) ? clone(runtime.resourcePoints) : [],
    currentCapacity: nonNegativeInt(runtime.currentCapacity, 0),
    maxCapacity: nonNegativeInt(runtime.maxCapacity, 0),
    city_def_val: nonNegativeInt(runtime.cityDefValue, 0),
    army_formation: Array.isArray(runtime.armyFormation) ? clone(runtime.armyFormation) : [],
    hospital: Array.isArray(runtime.hospital) ? clone(runtime.hospital) : [],
    defenseInfo: Array.isArray(runtime.defenseInfo) ? clone(runtime.defenseInfo) : [],
    careerInfo: Array.isArray(runtime.careerInfo) ? clone(runtime.careerInfo) : [],
    dayAndNight: clone(runtime.dayAndNight || {}),
    hireArmyShop: clone(runtime.hireArmyShop || {}),
    mine: clone(runtime.mine || {}),
    world_fortress: Array.isArray(runtime.worldFortress) ? clone(runtime.worldFortress) : [],
    recover_durability: clone(runtime.recoverDurability || {}),
    rescueCenterInfo: clone(runtime.rescueCenterInfo || {}),
    mixedInfo: clone(runtime.mixedInfo || {}),
    lottery: {
      times2: nonNegativeInt(lotteryRuntime.times2, 0),
      lotteryInfo: LAST_SHELTER_LOTTERY_REFERENCE.lotteryInfo,
      super: LAST_SHELTER_LOTTERY_REFERENCE.super,
      normal: LAST_SHELTER_LOTTERY_REFERENCE.normal,
      diamond: LAST_SHELTER_LOTTERY_REFERENCE.diamond,
      isNewLottery: LAST_SHELTER_LOTTERY_REFERENCE.isNewLottery,
      isOpen: LAST_SHELTER_LOTTERY_REFERENCE.isOpen,
      times1: nonNegativeInt(lotteryRuntime.times1, 0),
      todayCount: nonNegativeInt(lotteryRuntime.todayCount, 0),
      supermode: nonNegativeInt(lotteryRuntime.supermode, LAST_SHELTER_LOTTERY_REFERENCE.supermode),
      type: nonNegativeInt(lotteryRuntime.type, LAST_SHELTER_LOTTERY_REFERENCE.type),
      updatetime: nonNegativeInt(lotteryRuntime.updatetime, 0)
    },
    cargo: {
      rewardTime: nonNegativeInt(cargoRuntime.rewardTime, 0),
      rewardInfo:
...
function lastShelterVerifiedInitProjectionHazirla(state, options = {}) {
  const uid = String(options.uid == null ? "" : options.uid).trim();
  const city = state && state.lastShelterCityRuntime && typeof state.lastShelterCityRuntime === "object"
    ? state.lastShelterCityRuntime : {};
  const starter = state && state.lastShelterStarterAccountRuntime && typeof state.lastShelterStarterAccountRuntime === "object"
    ? state.lastShelterStarterAccountRuntime : {};
  const hero = state && state.lastShelterHeroRuntime && typeof state.lastShelterHeroRuntime === "object"
    ? state.lastShelterHeroRuntime : {};
  const missions = state && state.lastShelterMissionRuntime && typeof state.lastShelterMissionRuntime === "object"
    ? state.lastShelterMissionRuntime : {};
  ...
  const auxiliary = auxiliaryProjectionHazirla(state);
  const walletUser = goldInitProjectionHazirla(state) || {gold:0,gold1:0,paidGold:0};

  return {
    user: {uid,...walletUser},
    building: Array.isArray(city.buildings) ? clone(city.buildings) : [],
    buildingReference: buildingReferenceProjectionHazirla(),
    buildListConfig: Array.isArray(city.buildListConfig) ? clone(city.buildListConfig) : [],
    items: Array.isArray(starter.items) ? clone(starter.items) : [],
    queue: starterQueueInitProjectionHazirla(starter.queues),
    finishedQueue: Array.isArray(starter.finishedQueue) ? clone(starter.finishedQueue) : [],
    heroTemplates: clone(LAST_SHELTER_HERO_TEMPLATES),
    userGenerals: Array.isArray(hero.generals) ? clone(hero.generals) : [],
    troopTranList: runtimeTreesProjectionHazirla(state && state.troopTransferRuntime),
    fort: fortInitProjectionHazirla(state),
    truckInfo: initTruckProjectionHazirla(state),
    world: worldProjectionHazirla(state),
    store: storeProjectionHazirla(),
    missileList: missileInitProjectionHazirla(state),
    showScienceArray: showScienceArrayProjectionHazirla(),
    activity: activityReferenceProjectionHazirla(),
    firstPayReward: clone(LAST_SHELTER_FIRST_PAY_REWARD),
    onlineDuration: onlineDurationProjectionHazirla(state),
    helicopter_info: helicopterProjectionHazirla(state),
    task: taskProjectionHazirla(state),
    chapterTask: chapterTaskProjectionHazirla(state,uid),
    taskPoint: nonNegativeInt(missions.taskPoint,0),
    sevenDaysActivity: sevenDaysProjectionHazirla(state,uid),
    repayinfo: repayProjectionHazirla(state),
    alliance:
      alliance.alliance && typeof alliance.alliance === "object" && !Array.isArray(alliance.alliance)
...
module.exports = {
  LONG_MAX_STRING,
  buildingReferenceProjectionHazirla,
